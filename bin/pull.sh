...
# Example: ./bin/pull.sh staging -d http://murr.docker.localhost -u ./code/web/app
...
pullDatabase() {
  ...
  URL_SEARCH=$(kubectl get cm $APP_NAME-bedrock-project-wp-config -n $1 --no-headers -o custom-columns=":data.WP_HOME" --context $3 | sed 1q)
  URL_REPLACE=$2
...
pullUploads() {
  ...
  read -r -p "Proceed? [y/N] " response
  case "$response" in [yY][eE][sS] | [yY])
    kubectl cp $TARGET_POD:/var/www/html/web/app/uploads $2/uploads -n $1 --context $3
    ;;
  *)
